...
export default function UploadModal() {
  const { currentPath, loadFiles, addFileOptimistic } = useFiles();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Optimistic update for each file
      files.forEach(file => {
        const fileItem: any = {
          name: file.name,
          ...
          thumbnailUrl: null, // Will be updated on real load
          tenantId: tenantId || 'default'
        };
        addFileOptimistic(fileItem);
      });
      ...
      loadFiles(currentPath); // Refresh to get real data (e.g. thumbnails generated by server)
      refreshStorageInfo();
      handleClose();
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Upload failed', 'error');
      // If failed, loadFiles will revert optimistic updates naturally as it fetches current state
